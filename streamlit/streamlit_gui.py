...
<div class='tableauPlaceholder' id='viz1782154300066' style='position: relative'>
    ...
    <object class='tableauViz' style='display:none;'>
        ...
        <param name='path' value='shared/85FJMNKW2' />

        <param name='toolbar' value='no' id="toolbar"/>

        <param name='static_image' value='https://public.tableau.com/static/images/85/85FJMNKW2/1.png' />
...
<script type='text/javascript'>
    var divElement = document.getElementById('viz1782154300066');
...
def show_order_popup(selected_data, order_type):
    ...
    st.write(f"**Unit Price:** €{float(selected_data['UnitPrice']):.2f}")
    st.write(f"**Order Value:** €{abs(float(selected_data['Order_Value'])):.2f}")
    ...
    if order_type == "Purchased Orders":
        st.write(
            ...
            f"{selected_data['Description']} at €{float(selected_data['UnitPrice']):.2f} per item, "
            f"giving this order a value of €{abs(float(selected_data['Order_Value'])):.2f}."
        )
        ...
        st.write(
            ...
            f"{selected_data['Description']} at €{float(selected_data['UnitPrice']):.2f} per item, "
            f"meaning €{abs(float(selected_data['Order_Value'])):.2f} was refunded or lost from revenue."
        )
